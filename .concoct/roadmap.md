...
- Capability impact: makes accepted Product Owner judgment durable and able to advance the workflow from ready through roadmap reconciliation and work selection

### Outcome

Make the Product Owner an effective owner of product direction rather than a
read-only recommender whose accepted decisions are discarded operationally.

From `ready`, the Product Owner must be able to reconcile roadmap and accepted
capability evidence, decide whether a candidate is ready, select exactly one next
item, and cause that accepted decision to advance into planning. After delivery,
the Product Owner must be able to reconcile the completed item and current product
truth before selecting subsequent work. `status`, `exec`, and `run` must all observe
the same accepted decision and next action.

### Rationale

The current ready-state contract handcuffs the Product Owner. Its persona says it
owns the roadmap and decides which work is ready, but the `product-owner-next`
handoff is read-only, authorizes no updates, and requires the agent to recommend a
separate command rather than perform Product Owner work.

Observed invocations demonstrate that the Product Owner made sound decisions:

- with CON-039 `planned`, it returned `concoct plan CON-039` and explained why that
  item was the only structurally eligible critical work;
- with every item `candidate`, it returned `concoct roadmap` and explained that
  CON-039 should be reconciled into `planned` status.

Concoct accepted those results but did not promote either recommendation into
durable workflow evidence or a follow-up action. Repository evidence remained
unchanged, so reconciliation and `status` derived `Next: concoct next` again. A
repeat invocation consumed more allocation and was eventually rejected as a
duplicate or out-of-bounds recommendation.

This produces a loop:

```text
ready
→ invoke read-only Product Owner
→ accept semantic recommendation
→ discard its operational meaning
→ derive next action from unchanged evidence
→ ready / concoct next
```

The missing lifecycle boundary is acceptance and application of Product Owner
judgment. A structured recommendation is not merely output text: once validated
and approved where policy requires, it must become one-use decision evidence that
changes product state or authorizes the exact next transition.

### Product decisions

#### Product Owner authority

- The Product Owner owns roadmap content, prioritization, readiness, sequencing,
  and reconciliation against accepted current capabilities and archive evidence.
- `candidate` means that the Product Owner has not yet committed the item to
  planning; it does not make the item invisible to Product Owner evaluation.
- The Product Owner may refine a candidate and change it to `planned` when the
  outcome, scope, dependencies, prerequisites, acceptance criteria, and required
  product decisions are sufficiently complete.
- The Product Owner may update capability truth only when reconciling explicit
  accepted archive or delivery evidence. It must not manufacture acceptance,
  bypass Archivist obligations, or reinterpret unreviewed implementation as a
  capability.
- Routine roadmap reconciliation and selection are Product Owner work, not human
  intervention. Human intervention is reserved for a named product decision that
  cannot be resolved from authoritative evidence.

#### Ready-state decision model

The Product Owner returns exactly one semantic decision:

- `select` — identify one exact roadmap item to advance into planning;
- `reconcile-and-select` — apply authorized roadmap or capability reconciliation
  and identify one exact item to advance;
- `reconcile` — apply authorized reconciliation when no item can yet be selected;
- `human-decision-required` — stop for one specifically described product decision
  that authoritative project evidence cannot resolve; or
- `no-action` — report that no actionable or reconcilable recorded work exists.

A command string may be rendered for operator clarity, but it is not the durable
decision and must not be the only retained meaning of the outcome.

#### Durable decision evidence

- A validated Product Owner decision must be retained as executable-owned,
  evidence-bound, one-use workflow state until it is applied, rejected, superseded,
  or invalidated by repository drift.
- A selection record identifies the exact roadmap item, source evidence digest,
  Product Owner invocation and outcome, applicable approval, and intended next
  transition.
- Pending and accepted selection evidence must survive command exit so a later
  `status`, `exec`, or `run` observes the same decision without another model call.
- Applying a selection consumes the evidence exactly once. Planning activation,
  roadmap drift, changed prerequisites, a different active task, or a superseding
  Product Owner decision invalidates it explicitly rather than silently replaying
  it.
- The exact repository representation is a planning decision, but accepted product
  selection must be derivable without conversation history or an ephemeral adapter
  result alone.

#### Approval and application

- Existing `next` approval policy protects application of Product Owner selection
  and reconciliation, not the ability of the Product Owner to perform its analysis.
- When approval is required, Concoct retains the proposed mutations, selected item,
  rationale, evidence digest, and exact consequences, then stops before applying
  them.
- `concoct run --approve next` applies only the retained evidence-bound decision;
  it does not re-invoke the Product Owner.
- When approval is not required, a validated Product Owner decision may be applied
  immediately through executable-owned validation and commit authority.
- Approved `select` or `reconcile-and-select` proceeds directly into the planning
  transition, subject to the separately configured planning gate. It must not
  return to `concoct next` between selection and planning.

#### Mutation and validation boundary

- Product Owner mutations are limited to the roadmap and capability reconciliation
  explicitly authorized for the ready-state action.
- The executable validates stable identifiers, status transitions, dependency and
  prerequisite references, archive provenance, reserved identifiers, supported
  origins, capability truth, selected-item uniqueness, and absence of unauthorized
  changes before accepting the decision.
- Invalid Product Owner mutations are preserved and diagnosed as supervised
  candidates; they are never silently accepted, discarded, or reported as durable
  progress.
- Deterministic selection evidence and executable bookkeeping are created by
  Concoct, not fabricated by the agent.

#### Ready-to-ready reconciliation

- Returning to `ready` after integration retains a clear completed-transition
  result and enough accepted evidence for the Product Owner to reconcile delivered
  work.
- The next Product Owner action may remove a delivered item from the outstanding
  roadmap after verifying archive, integration, capability, dependency, and
  identifier-reservation evidence.
- The same Product Owner action may then select subsequent work when reconciliation
  leaves an eligible item.
- Current-state policy eligibility must not overwrite or obscure the report that
  the preceding task completed successfully.
- A clean ready state with no active task must report planning, development,
  review, archival, and integration as inactive or not currently applicable—not as
  blocked work or as a non-Git active task.

#### Cost and duplicate protection

- Concoct must check for a retained applicable Product Owner decision before making
  another agent invocation.
- An unchanged accepted or pending decision is rendered and resumed
  deterministically; it is not regenerated by the model.
- A completed recommendation that causes no durable progress is not classified as
  accepted cost unless it results in retained decision evidence, a valid
  intervention, or an applied Product Owner mutation.
- Duplicate and bound detection must occur before a billable invocation whenever
  unchanged evidence already proves that the same Product Owner action cannot
  advance.

### Requirements

- Replace the operationally incomplete read-only ready-state recommendation
  boundary with an executable Product Owner decision boundary that can authorize
  and validate roadmap/capability mutations and exact work selection.
- Preserve a read-only inspection or prompt-rendering mode for humans who want to
  examine Product Owner guidance without applying it, but do not use that mode as
  the autonomous `exec`/`run` lifecycle action.
- Make candidate items available as authoritative evidence for Product Owner
  readiness evaluation while retaining structural planning validation for the
  item ultimately selected.
- Accept Product Owner selection of an already `planned` item without requiring a
  roadmap mutation.
- Allow a Product Owner to refine and promote a candidate as part of one
  `reconcile-and-select` action.
- Persist proposed and accepted decisions with source-evidence binding and expose
  them through status and explanation output.
- Apply the configured `next` approval to the retained decision rather than to a
  fresh model invocation.
- After decision application, render or execute the exact resulting planning action
  without another Product Owner call.
- Reconcile roadmap, capability, archive, integration, and identifier evidence at
  the end of a delivered task before reporting the next product action.
- Ensure `status`, `next`, `exec`, and `run` use one authoritative next-action
  derivation and cannot disagree merely because semantic adapter output was stored
  only in an invocation record.
- Show the Product Owner's decision, rationale, proposed and applied changes,
  approval state, durable effect, resulting workflow state, and exact next action
  in operator-facing output.
- Retain agent-cost measurement and accepted/wasted disposition across Product
  Owner recommendation, reconciliation, approval, application, invalidation, and
  reuse.

### Scope boundaries

- CON-040 repairs Product Owner authority and ready-state advancement. It does not
  implement general retained-candidate recovery for Developer, Reviewer, or
  Archivist finalization; that remains CON-039.
- It does not provide crash-resumable multi-action coordination or permanent run
  history; those remain CON-034.
- It does not remove human product authority. Configured approval gates still
  protect meaningful selection and reconciliation decisions.
- It does not allow the Product Owner to change capability truth without accepted
  archive/delivery evidence or to bypass review, archival, or integration.
- It does not automate remote push or other external delivery authority.

### Acceptance criteria

- From `ready` with CON-039 as a suitable `candidate`, the Product Owner can refine
  or promote it, select it, and leave an evidence-bound decision awaiting approval
  without manual roadmap editing.
- Approving that retained decision does not re-invoke the Product Owner; it applies
  the exact approved changes and proceeds to planning, subject only to the planning
  gate.
- From `ready` with CON-039 already `planned`, one Product Owner invocation selects
  it and causes `status`, `exec`, and `run` to report or perform `concoct plan
  CON-039` rather than returning to `concoct next`.
- A Product Owner `roadmap` or reconciliation outcome results in validated retained
  mutations or a specifically named human decision; it is never accepted merely as
  advice that the operator should run another Product Owner command.
- `adapter-result`, reconciliation output, `status`, `exec`, and `run` agree on the
  accepted Product Owner decision and resulting next action.
- Repeating a command against unchanged evidence reuses or reports the pending
  decision without making another billable Product Owner invocation.
- Repository drift invalidates pending selection clearly and safely; stale or
  superseded decisions cannot be applied.
- Invalid roadmap or capability mutations are rejected with actionable diagnostics
  and without unauthorized durable changes.
- After successful task integration, the completed transition remains visible,
  delivered roadmap and capability evidence can be reconciled, and the workflow
  can select subsequent work or report a genuine decision/no-action outcome.
- A clean ready state never reports absent task phases as blocked or claims that a
  nonexistent active task is not Git-backed.
- Tests reproduce both retained 2026-08-18 failures: the accepted `plan CON-039`
  recommendation that reverted to `concoct next`, and the accepted `concoct
  roadmap` recommendation that was ignored by later status derivation.
- Full tests, race tests, vet/static checks, native and Windows builds, fresh
  initialization, source/template parity, shell validation, and focused diff
  inspection pass.
